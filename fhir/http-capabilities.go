...
func initHTTPCapabilities() {
	slip.Define(
		func(args slip.List) slip.Object {
			f := HTTPCapabilities{Function: slip.Function{Name: "http-capabilities", Args: args}}
			f.Self = &f
			return &f
		},
		&slip.FuncDoc{
...
"/metadata" suffix is appended to the URL path.
...
retrieved, a CapabilityStatement, nil, or OperationOutcome. The last element in the list are
the headers. Note: For non-conformant FHIR servers a Bundle or other data may be returned.


For additional information about the FHIR HTTP read refer
to https://www.hl7.org/fhir//http.html#capabilities.
`,
		}, &Pkg)
}

// HTTPCapabilities represents the http-capabilities function.
